...
Esta tabla representa a todas las personas que forman parte de la organización (Administradores, Supervisores y Agentes). Los agentes no cuentan con credenciales de acceso, por lo que sus campos de login aceptan valores nulos (`NULL`). Se ha removido por completo el campo `phone` de esta tabla para asegurar que los teléfonos operativos se almacenen de forma exclusiva en la tabla de dispositivos.
...
*   `foto` (TEXT): URL o ruta de almacenamiento de la foto de perfil del usuario.
...
*   `assigned_phone` (TEXT, NOT NULL, UNIQUE): Número de línea telefónica de la empresa asociada al dispositivo. **Este es el único campo de la base de datos donde se almacena el número de teléfono del agente**.
...
```
  ...
  |    | initials                  |             +-------------------------------+
  |    | online                    |             | PK | id                       |
  |    | created_at                |             | FK | agent_id (-> users.id)   |
  |    | updated_at                |             |    | client_name              |
  |    |                           |             |    | topic                    |
  +--------------------------------+             +-------------------------------+
```
...
4.  **Cumplimiento Estricto de Reglas de Negocio:** Al eliminar campos innecesarios como `assigned_at` y el campo `phone` redundante en `users`, se simplifica el modelo físico facilitando consultas de base de datos más ágiles e intuitivas.
5.  **Estandarización del Idioma:** Se renombró el campo `avatar` por `foto` en la tabla `users` para ajustarse perfectamente al idioma español del sistema.
