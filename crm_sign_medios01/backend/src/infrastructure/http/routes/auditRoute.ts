...
auditRouter.get('/audit-logs', requireAuthAndAuditAccess, async (_req, res) => {
  try {
    ...
    const rows = await db.query(
      'SELECT id, entity_type AS "entityType", entity_id AS "entityId", action, performed_by AS "performedBy", details, created_at AS "createdAt" FROM audit_logs ORDER BY created_at DESC LIMIT 100',
    );
